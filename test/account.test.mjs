...
describe("account resources", () => {
  it("cards() reads a bare JSON array payload", async () => {
    ...
    const cards = await client.account.cards();
  ...
  it("cardLeftToSpend() requires and sends the card id", async () => {
    ...
    const r = await client.account.cardLeftToSpend("card-123");
  ...
  it("transactions() returns the cursor page verbatim and forwards params", async () => {
    ...
    const page = await client.account.transactions({ includeDustReceives: true, limit: 2 });
  ...
  it("iterateTransactions() follows next_cursor until has_more is false", async () => {
    ...
    const ids = [];
    for await (const tx of client.account.iterateTransactions({ limit: 2 })) ids.push(tx.id);
  ...
  it("iterateTransactions() stops when has_more is true but no cursor is provided", async () => {
    ...
    const ids = [];
    for await (const tx of client.account.iterateTransactions()) ids.push(tx.id);
